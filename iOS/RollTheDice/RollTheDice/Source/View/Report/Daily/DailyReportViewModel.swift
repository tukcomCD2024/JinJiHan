...
class DailyReportViewModel: ObservableObject{
    @Published var dailyReportList: DailyReportList
    
    /// 일주일 평균 조회수 
    var averageView: String {
        var aver = 0.0
        
        for daily in dailyReportList.reportList {
    ...
    init(
        dailyReportList: DailyReportList = .init(
            reportList:
        ...
        self.dailyReportList = dailyReportList
    }
}
